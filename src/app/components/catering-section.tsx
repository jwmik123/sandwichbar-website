      ...
      <div className="mx-auto max-w-6xl flex flex-col lg:flex-row items-center gap-16">
        ...
        <div className="flex flex-col gap-8 lg:w-3/5">
          <div className="flex flex-col gap-2">
            {data.label && (
              <span
                className="text-sm font-semibold uppercase tracking-widest text-cream/50"
                data-sanity={attr.scope('label').toString()}
              >
                {data.label}
              </span>
            )}
            {data.title && (
              <h2
                className="text-4xl md:text-5xl font-bold tracking-tight text-cream uppercase leading-tight"
          ...
          {data.body && (
            <div
              className="flex flex-col gap-5 text-cream/75 text-lg leading-relaxed [&>p]:m-0"
          ...
          <div className="flex flex-wrap gap-4 pt-2">
            {data.orderNowText && (
              <TransitionLink
                ...
                data-sanity={attr.scope('orderNowText').toString()}
              >
                {data.orderNowText}
              </TransitionLink>
            )}
            {data.freeTestLunchText && (
              <TransitionLink
                ...
                data-sanity={attr.scope('freeTestLunchText').toString()}
              >
                {data.freeTestLunchText}
              </TransitionLink>
            )}
